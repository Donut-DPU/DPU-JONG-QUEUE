...
const totalPages = computed(() => {
  ...
  return Math.ceil(
    rows.value.length / itemsPerPage
  )
})
...
async function load() {
  ...
  try {
    ...
    if (filter.value.date) {
      q.set('date', filter.value.date)
    }

    if (filter.value.serviceId) {
      q.set(
        'serviceId',
...
async function setStatus(b, status) {
  ...
  try {
    ...
    dialogMessage.value =
      e.message || 'อัปเดตสถานะไม่สำเร็จ'
